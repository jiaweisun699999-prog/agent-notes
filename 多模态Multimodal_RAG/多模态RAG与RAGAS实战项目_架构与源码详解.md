...
- **解决方案**：在 [db_operator.py](file:///d:/A大模型/大模型课件资料/《多模态RAG+RAGAS实战项目》资料（课时12-21）/代码/Multimodal_RAG/milvus_db/db_operator.py) 中，系统会自动提取图片在 Markdown 中的**前文段落 (`prev_text`)** 和 **后文段落 (`next_text`)**，连同图片的 Base64 编码一并发送给 Vision LLM：
  ```python
  # 构建多模态上下文 Prompt
...
- **解决方案**：在 [workflow.py](file:///d:/A大模型/大model/大模型课件资料/《多模态RAG+RAGAS实战项目》资料（课时12-21）/代码/Multimodal_RAG/graph/workflow.py) 中配置 `interrupt_before=['human_approval']`。当 RAGAS 自评估分数未达到预期时，系统自动挂起，等待管理员在界面操作 `approve`（批准）或 `rejected`（拒绝并触发联网搜索降级）。
