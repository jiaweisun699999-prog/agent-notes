> 📌 **[AI 大模型与云原生全栈知识库](./README.md)** / **34. 多模态 RAG 与 RAGAS 全栈进阶与源码解析手册**
...
在 [markdown_parser.py](./《RAG企业知识库项目》课程代码/RAG_PROJECT/documents/markdown_parser.py) 中，系统通过字典树层级算法实现了标题拼接：
...
在 [dots_ocr/parser.py](./多模态Multimodal_RAG/dots_ocr/parser.py) 中，`do_parse` 函数负责将原始 PDF 解析为逐页 Markdown 文件并抽取内嵌图像：
...
`MarkdownDirSplitter` ([splitters/splitter_md.py](./多模态Multimodal_RAG/splitters/splitter_md.py)) 包含三大核心能力：
...
单独的图片如果缺乏文字描述，在纯向量库中极难被检索到。在 [milvus_db/db_operator.py](./多模态Multimodal_RAG/milvus_db/db_operator.py) 的 `generate_image_description` 中，通过抽取当前图片在文档中的**前文 (`prev_text`)** 与 **后文 (`next_text`)**，传给多模态大模型 (`qwen3-vl-plus`) 生成限制在 300 字内的精炼描述：
...
在 [milvus_db/collections_operator.py](./多模态Multimodal_RAG/milvus_db/collections_operator.py) 中，定义了支持混合检索的多模态 Collection：
...
在 [milvus_db/db_retriever.py](./多模态Multimodal_RAG/milvus_db/db_retriever.py) 中，检索器逻辑如下：
...
在 [my_llm.py](./多模态Multimodal_RAG/my_llm.py) 中统一管理所有的模型实例：
...
[utils/embeddings_utils.py](./多模态Multimodal_RAG/utils/embeddings_utils.py) 使用 `multimodal-embedding-v1` 模型输出 1024 维度的 Dense 向量。
...
在 [graph/my_state.py](./多模态Multimodal_RAG/graph/my_state.py) 中，继承 `MessagesState` 扩展业务字段：
...
在 [graph/all_router.py](./多模态Multimodal_RAG/graph/all_router.py) 中定义了工作流的状态条件转移逻辑：
...
[evaluate/evaluate_self.py](./多模态Multimodal_RAG/evaluate/evaluate_self.py) 使用 `ragas` 官方 SDK 计算客观性能指标：
...
[main.py](./多模态Multimodal_RAG/main.py) 通过 Gradio 搭建一站式用户交互台：
- 动态 PDF 上传与解析进度显示。
