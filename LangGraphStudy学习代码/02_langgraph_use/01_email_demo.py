...
#====== 定义节点 ======
def classify_email(state: EmialState) -> Command[Literal["search_info","email_reply"]]:
...
def search_info(state: EmialState) -> Command[Literal["email_reply"]]:
    "根据邮件分类，搜索相关文档"
...
def email_reply(state: EmialState) -> Command[Literal["review",END]]:
    "根据搜索结果，生成邮件回复"
    #原始邮件
    ...
    if urgency == "high":
        return Command(update={"email_response":response.content},goto="review")

    return Command(update={"email_response":response.content},goto=END)



# def review_email_reply(state: EmialState)-> Command[Literal[END]]:
#     "模拟人工审核"
