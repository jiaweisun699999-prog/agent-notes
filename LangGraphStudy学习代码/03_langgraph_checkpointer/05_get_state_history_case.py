"""
LangGraph中Checkpointer使用
"""
import operator
from typing import TypedDict, Annotated

from langgraph.checkpoint.memory import InMemorySaver
from langgraph.constants import START, END
from langgraph.graph import StateGraph


#1. 定义状态
class State(TypedDict):
    foo: str
    bar: Annotated[list[str],operator.add]


#2. 定义节点
def node_a(state: State):
    """节点A:写入 foo=a， bar中追加a"""
    return {"foo": "a", "bar": ["a"]}

def node_b(state: State):
    """节点B:写入 foo=b， bar中追加b"""
    return {"foo": "b", "bar": ["b"]}

#3.构建图
builder = StateGraph(State)
builder.add_node("node_a",node_a)
builder.add_node("node_b",node_b)

builder.add_edge(START,"node_a")
builder.add_edge("node_a","node_b")
builder.add_edge("node_b",END)

#4. 编译图
checkpointer = InMemorySaver()
graph = builder.compile(checkpointer=checkpointer)


# 输出Graph 图/工作流为 PNG
png_data = graph.get_graph().draw_mermaid_png()
with open("graph.png", "wb") as f:
    f.write(png_data)
print("图片已保存到 graph.png")


#5. 运行图
config = {"configurable": {"thread_id": "thread_1"}}

result = graph.invoke({"foo": "", "bar": []},config=config)
print("result:",result)

#获取checkpointer 最新的状态
lastest_state = graph.get_state(config)
# print(type(lastest_state))
# print("lastest_state:",lastest_state)

# 获取历史所有的checkpointer
state_history = list(graph.get_state_history(config))

node_a_checkpint =None

for snapshot in state_history:
    print("snapshot:",snapshot)
    if snapshot.next == ('node_b',):
        node_a_checkpint = snapshot


print(f"node_a_checkpint:{node_a_checkpint}")



#
#
# for snapshot in graph.get_state_history(config):
#     print(type(snapshot))
#     print("snapshot:",snapshot)
#
#     # print(f"step={snapshot.metadata['step']},next={snapshot.next},values={snapshot.values}")
#     #
#     # if snapshot.parent_config is None:
#     #     print("这是根节点")
#
#
# parent_ck_id = lastest_state.parent_config["configurable"]["checkpoint_id"]
#
#
# new_config = {
#     "configurable": {
#         "thread_id": "thread_1",
#         "checkpoint_id":parent_ck_id
#     }
# }
#
# #{'foo': 'a', 'bar': ['a']}
# new_state = graph.get_state(new_config)
# print("new_state:",new_state)






